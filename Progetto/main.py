...
for item in json_array:  #per ogni item nella risposta json
    otherPosts.append(item) #inserisco in una struttura

print("len {}".format( len(otherPosts)))

myposts_sorted = sorted(otherPosts, key=lambda k: #ordino per numero like
...
#filtro foto semplici ( non video , non post multipli)
while i  < len(top_posts):
 #print("Json = {}".format(i))
     print(i)
     try:  
         photo = top_posts[i]["image_versions2"]["candidates"][0]["url"]   #prendo l'url
         reList.append(photo) #struttura contenenti URLs
         i+=1
     except KeyError :
...
#Creo delle foto con gli oggetti evidenziati, con dei rettangoli e delle percentuali 
#( si riferiscono al grado di affidamento della detection)
i = 0
...
singleWeigth=int(100/numOfRows)  #peso singolo distribuito in base al numero di oggetti
...
#oggetti con uguali like avranno peso uguale
peso=singleWeigth
while i < numOfRows:
    doppione=False
...
#calcolo il valore finale in base a gli oggetti che ho inserito
while i < len(myObs):
